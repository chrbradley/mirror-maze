...
export class Controls {
  ...
  private drawSlider() {
    ...
    p.noFill();
    p.line(this.sliderX, this.sliderY + 20, this.sliderX + this.sliderWidth, this.sliderY + 20);
    ...
    // Slider handle
    const handleX = this.sliderX + (this.bounceCount / this.maxBounces) * this.sliderWidth;
    p.fill(COLORS.CYAN);
    p.noStroke();
    ...
    p.textSize(16);
    p.text(`Bounces: ${this.bounceCount}`, this.sliderX + this.sliderWidth / 2, this.sliderY - 5);
  }
  ...
  private drawModeButton() {
    ...
    p.strokeWeight(STROKE_WEIGHTS.MEDIUM);
    p.noFill();
    p.rect(this.modeButtonX, this.modeButtonY, this.modeButtonWidth, this.modeButtonHeight);
    ...
    p.textSize(16);
    const modeText = this.mode === 'sandbox' ? 'Mode: Sandbox' : 'Mode: Challenge';
    p.text(modeText, this.modeButtonX + this.modeButtonWidth / 2, this.modeButtonY + this.modeButtonHeight / 2);
  }
  ...
  private drawFireButton() {
    ...
    p.strokeWeight(STROKE_WEIGHTS.MEDIUM);
    p.noFill();
    p.rect(this.fireButtonX, this.fireButtonY, this.fireButtonWidth, this.fireButtonHeight);
    ...
    p.textAlign(p.CENTER, p.CENTER);
    p.textSize(20);
    p.text('FIRE!', this.fireButtonX + this.fireButtonWidth / 2, this.fireButtonY + this.fireButtonHeight / 2);
  }
  ...
  handleMousePressed(mouseX: number, mouseY: number) {
    ...
    if (mouseY >= this.sliderY + 10 && mouseY <= this.sliderY + 30 && dist < 20) {
      return true; // Will handle in drag
    }
    
    // Check mode button
    if (this.isInButton(mouseX, mouseY, this.modeButtonX, this.modeButtonY, this.modeButtonWidth, this.modeButtonHeight)) {
      this.toggleMode();
      return true;
    }
    ...
    if (this.mode === 'challenge' && 
        this.isInButton(mouseX, mouseY, this.fireButtonX, this.fireButtonY, this.fireButtonWidth, this.fireButtonHeight)) {
      this.onFire?.();
      return true;
    }
  ...
  handleMouseDragged(mouseX: number, mouseY: number) {
    ...
    if (mouseY >= this.sliderY && mouseY <= this.sliderY + 40) {
      const newValue = Math.round(((mouseX - this.sliderX) / this.sliderWidth) * this.maxBounces);
  ...
  private isInButton(mouseX: number, mouseY: number, buttonX: number, buttonY: number, buttonWidth: number, buttonHeight: number): boolean {
